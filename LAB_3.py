...
def hybrid_data_encryption(settings: dict) -> None:
    ...
    dc_symmetric_key = d_private_key.decrypt(content, padding.OAEP( mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=None))
    print('расшифрованный ключ')
    print(dc_symmetric_key)

    with open(settings['initial_file'], 'w') as fp:
        json.dump('Hello!', fp)
    ...
    padder = padding2.ANSIX923(128).padder()
    ...
    iv = os.urandom(16)
    cipher = Cipher(algorithms.SM4(dc_symmetric_key), modes.CBC(iv))
...
def hybrid_data_decryption(settings: dict) -> None:
    ...
    print(dc_text)
    print(dc_text.decode(encoding))
    print('расшифрованный текст')
    print(unpadded_dc_text)
    print(unpadded_dc_text.decode(encoding))
